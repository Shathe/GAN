...
def conv2d_bn(x, filters, num_row, num_col, padding='same', strides=(1, 1), dilation_rate=(1, 1), training=True, last=False, layer_index=0):
    ...
    with tf.variable_scope('conv2d_bn_'+str(layer_index)):
        ...
        x = tf.layers.conv2d(x, filters*4, (num_row, num_col), strides=(1,1), padding=padding, dilation_rate=dilation_rate,activation=activation_fn,   bias_initializer=None,
                 kernel_initializer=tf.contrib.layers.xavier_initializer_conv2d(), kernel_regularizer=None, name='conv_bottle'+ str(layer_index)) # there is also dilation_rate!
        ...
        x1 = tf.layers.conv2d(x, n_filter+extra_filters, (num_row, num_col), strides=strides, padding=padding, dilation_rate=dilation_rate,activation=activation_fn,   bias_initializer=None,
        ...
        x2 = tf.layers.conv2d(x, n_filter, (5,5), strides=strides, padding=padding, dilation_rate=dilation_rate,activation=activation_fn,   bias_initializer=None,
        ...
        x3 = tf.layers.conv2d(x, n_filter, (num_row, num_col), padding=padding, dilation_rate=(5,5),activation=activation_fn,   bias_initializer=None,
        ...
        x3 = tf.layers.conv2d(x3, n_filter, (num_row, num_col),strides=strides,  padding=padding, dilation_rate=dilation_rate,activation=activation_fn,   bias_initializer=None,
...
#hacer capas profundas con poca kernels, tambien hacer con dilataicones y con saltos
    ...
    layer_index = increment(layer_index)
    print(x.get_shape())
    x = conv2d_simple(x, 164, 3, 3, padding='same', strides=(1, 1), training=training, layer_index=layer_index)
    print(x.get_shape())
    layer_index = increment(layer_index)
    x = conv2d_simple(x, 164, 3, 3, padding='same', strides=(1, 1), training=training, layer_index=layer_index)
    print(x.get_shape())
    ...
    print(x.get_shape())
    layer_index = increment(layer_index)
    x3 = conv2d_simple(x, 164, 3, 3, padding='same', strides=(1, 1), training=training, layer_index=layer_index)
    x = tf.layers.average_pooling2d(x3, pool_size=(2, 2), strides=(2, 2), name='pool'+ str(layer_index))
    layer_index = increment(layer_index)
    print(x.get_shape())
    x = conv2d_simple(x, 164, 3, 3, padding='same', strides=(1, 1), training=training, layer_index=layer_index)
    ...
    print(x.get_shape())
    layer_index = increment(layer_index)
    x2 = conv2d_simple(x, 164, 3, 3, padding='same', strides=(1, 1), training=training, layer_index=layer_index)
    x = tf.layers.average_pooling2d(x2, pool_size=(2, 2), strides=(2, 2), name='pool'+ str(layer_index))
    layer_index = increment(layer_index)
    print(x.get_shape())
    layer_index = increment(layer_index)
    x = conv2d_simple(x, 192, 3, 3, padding='same', strides=(1, 1), training=training, layer_index=layer_index)
    layer_index = increment(layer_index)
    x = conv2d_simple(x, 192, 3, 3, padding='same', strides=(1, 1), training=training, layer_index=layer_index)
    layer_index = increment(layer_index)
    x = conv2d_simple(x, 192, 3, 3, padding='same', strides=(1, 1), training=training, layer_index=layer_index)
    ...
    x = deconv_simple(x, 192, 3, 3, padding='same', strides=(2, 2), training=training, layer_index=layer_index)
    layer_index = increment(layer_index)
    x = tf.concat([x, x2], axis=3)
    print(x.get_shape())
    x = conv2d_simple(x, 192, 3, 3, padding='same', strides=(1, 1), training=training, layer_index=layer_index)
    layer_index = increment(layer_index)
    x2_ = conv2d_simple(x, 192, 3, 3, padding='same', strides=(1, 1), training=training, layer_index=layer_index)
    layer_index = increment(layer_index)

    x = deconv_simple(x2_, 164, 3, 3, padding='same', strides=(2, 2), training=training, layer_index=layer_index)
    layer_index = increment(layer_index)
    x = tf.concat([x, x3], axis=3)
    print(x.get_shape())
    x = conv2d_simple(x, 164, 3, 3, padding='same', strides=(1, 1), training=training, layer_index=layer_index)
    layer_index = increment(layer_index)
    x3_ = conv2d_simple(x, 164, 3, 3, padding='same', strides=(1, 1), training=training, layer_index=layer_index)
    layer_index = increment(layer_index)


    x = deconv_simple(x3_, 164, 3, 3, padding='same', strides=(2, 2), training=training, layer_index=layer_index)
    layer_index = increment(layer_index)
    x = tf.concat([x, x4], axis=3)
    print(x.get_shape())
    x = conv2d_simple(x, 164, 3, 3, padding='same', strides=(1, 1), training=training, layer_index=layer_index)
    layer_index = increment(layer_index)
    x4_ = conv2d_simple(x, 164, 3, 3, padding='same', strides=(1, 1), training=training, layer_index=layer_index)
    layer_index = increment(layer_index)
    ...
    layer_index = increment(layer_index)
    x4_ = deconv_simple(x4_, 164, 3, 3, padding='same', strides=(4, 4), training=training, layer_index=layer_index)
    layer_index = increment(layer_index)
    x3_ = deconv_simple(x3_, 164, 3, 3, padding='same', strides=(8, 8), training=training, layer_index=layer_index)
    layer_index = increment(layer_index)
    x2_ = deconv_simple(x2_, 192, 3, 3, padding='same', strides=(16,16), training=training, layer_index=layer_index)
    layer_index = increment(layer_index)
